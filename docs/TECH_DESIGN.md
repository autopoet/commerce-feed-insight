...
项目定位是面试前快速完成的小型学习项目，不做企业级复杂架构，但需要体现清晰的工程拆分、类型设计、状态流转、性能优化和埋点设计意识。
...
- 复杂虚拟列表库：可以先自实现固定高度虚拟列表，方便面试讲清楚原理。
...
- `AdEntryPage`：选择广告上下文，进入推荐流。
- `FeedPage`：展示商品推荐流，触发曝光、点击、加购、购买事件。
- `DashboardPage`：消费事件流，展示漏斗指标、维度对比和事件日志。
