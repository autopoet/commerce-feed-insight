...
- 响应式适配：推荐流移动端使用紧凑左图右文商品卡，并通过媒体查询同步调整虚拟列表 `itemHeight`。
...
MVP 使用固定高度虚拟列表，便于快速实现和讲清楚。桌面端和移动端分别使用不同固定行高，避免移动端卡片压缩后虚拟列表仍保留桌面行距。

核心参数：

- `itemHeight`：商品卡片固定高度，桌面端为 `244px`，移动端为 `176px`。
...
- 看板指标可以使用简洁块状布局。
- 商品流保留足够图片面积，体现电商场景。
- 移动端推荐流保留左图右文商品卡，图片缩小为侧栏，避免一屏只能看到大图堆叠。
- 移动端虚拟列表行高由 `matchMedia('(max-width: 860px)')` 驱动，和 CSS 卡片高度保持一致。
...
- 看板指标是否来自事件流。
- 清空数据是否同时清空曝光去重集合。
- 移动端推荐流商品卡是否保持紧凑两列布局，图片宽度不超过主要内容区。
...
- 推荐流新增移动端紧凑布局：商品卡保持左图右文，避免单列大图堆叠。
- 虚拟列表行高根据视口宽度响应式切换，桌面端 `244px`，移动端 `176px`。
- Playwright 增加移动端推荐流布局断言，并生成 `docs/screenshots/04-mobile-feed.png`。
