...
### 9.1.1 MVP PV / UV 口径说明

当前 MVP 不单独统计 UV 指标，核心看板指标以事件流聚合为主：

| 指标 | 当前口径 | 说明 |
| --- | --- | --- |
| 曝光数 | session 内商品去重后的有效曝光事件数 | 同一 session 内同一商品只记录一次 `product_exposure` |
| 点击数 | `product_click` 事件次数，偏 PV 口径 | 同一用户连续点击同一商品会产生多条点击事件 |
| 加购数 | `add_to_cart` 事件次数，偏 PV 口径 | MVP 不做加购去重，连续点击会产生多条加购事件 |
| 购买数 | `purchase` 事件次数，偏 PV 口径 | MVP 为模拟购买，不做库存、支付状态和订单幂等校验 |

术语说明：

- PV 口径关注行为发生次数，同一用户重复触发会重复计数。
- UV 口径关注发生行为的独立用户数，同一用户重复触发通常只计一次。
- 当前项目优先表达“事件采集 -> 归因 -> 漏斗聚合”的前端链路，因此暂不拆分点击 UV、加购 UV、购买 UV。
...
- 加购次数 / 加购用户数。
- 购买事件 / 支付订单数 / 支付用户数。
- 商品维度去重、用户维度去重和 session 维度去重的具体边界。
